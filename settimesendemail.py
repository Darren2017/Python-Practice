...
from celery.utils.log import get_task_logger            '''记录器,似乎可以不用，或者说我还没理解到位'''
...
celery = Celery('settimesendemail', broker=CELERY_BROKER_URL, backend=CELERY_BACKEND_URL)   '''创建celery实例'''
mails = Mail(app)           '''邮件....'''
...
def setup_periodic_tasks(sender, **kwargs):         '''由 celery beat实现周期性调用'''
	...
	sender.add_periodic_task(10.0,send_async_email.s(msg_dict2('recipientaddress',' 一封邮件')))
